#ifndef SIMPLE_BUILTINS_H
#define SIMPLE_BUILTINS_H
...
int mysh_launch(int in, int out, cmd_struct* command,int final,int sockfd);
int mysh_file_redirection(int in,cmd_struct* cmd,char* filename,int sockfd);
int mysh_printenv(char **args,int sockfd);
int mysh_setenv(char **args,int sockfd);
int mysh_exit(char **args,int sockfd);
int mysh_number_pipe(cmd_struct* command,int in, int read , int write, char symbol, char* number , int** table,int sockfd);
