...
void parse_query_string(std::string query_string){
    ...
    std::getline(myStream,server_info,'&');
    sh = server_info.substr(3,server_info.length()-3);
    // std::cout<<"socks host= "<<sh<<std::endl;

    server_info = "";

    std::getline(myStream,server_info,'&');
    sp = server_info.substr(3,server_info.length()-3);
    // std::cout<<"socks port= "<<sp<<std::endl;
...
void getOutputFromNPserver(int client_id,std::vector<std::string> input){
    // std::cout << "getting output from np server" <<std::endl;
    ...
    [input,client_id](boost::system::error_code ec,std::size_t length){
        // std::cout<<"in the async read"<<std::endl;
        if(!ec){
          ...
          // std::cout<< "receive some error code but not eof" <<std::endl;
          if(ec == boost::asio::error::eof){
            // std::cout<< "receive eof and close socket "<<std::endl;
            clients_info[client_id].tcp_socket.close();
          }
          return ;
        }
    }
...
void getSocksReply(int client_id){
  clients_info[client_id].tcp_socket.async_read_some(boost::asio::buffer(clients_info[client_id].read_buffer,8),
      [client_id](boost::system::error_code ec, std::size_t length){
        if(ec || clients_info[client_id].read_buffer[0] != 0 || clients_info[client_id].read_buffer[1]!=90){
          clients_info[client_id].tcp_socket.close();
          std::cout<<"socks_connection_closed"<<std::endl;
          return;
        }
        
        std::vector<std::string> input = parse_txt_input(clients_info[client_id].file_to_open);
...
int main(int argc, char* argv[]){
    // std::cout << "connect to cgi" <<std::endl;
    ...
    for(;it!=clients_info.end();it++){
        ...
        sock_req += (char)4;
        sock_req += (char)1;
        sock_req += (char)(stoi(it->server_port_num) /256);
        sock_req += (char)(stoi(it->server_port_num) %256);
        sock_req += (char)0;
        sock_req += (char)0;
        sock_req += (char)0;
        ...
        sock_req += (char)0;
        sock_req = sock_req + it->server_hostname + (char)0;
        // std::cout << "sock_req = "<<sock_req <<std::endl;

        boost::asio::ip::tcp::resolver resolver(console_io_context);
        
        boost::asio::ip::tcp::resolver::query my_query(sh, sp);
        ...
        boost::asio::async_write(it->tcp_socket, boost::asio::buffer(sock_req),
        [i](boost::system::error_code ec, std::size_t len){
				  if (ec) return;
          getSocksReply(i);
			  });
    i++;
